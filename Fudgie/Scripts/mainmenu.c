...
static const Uint16 buttonGap =  (SCREEN_WIDTH-(buttonW*BUTTONAMOUNT))/(BUTTONAMOUNT+1);
static Texture titleText;
static Button playerAmountButtons[BUTTONAMOUNT];

static void mainmenuscene_loadAssets(context *ctx) {
   Texture_init_andLoadFromRenderedText(&titleText, ctx->renderer, ctx->gFont, (SDL_FRect){((float)SCREEN_WIDTH/2)-(float)SCREEN_WIDTH/4,(float)SCREEN_HEIGHT/6,(float)SCREEN_WIDTH/2,(float)SCREEN_WIDTH/4}, "Fudgie!", 7, (SDL_Color){255,255,255,255});
   for (int i = 0; i<BUTTONAMOUNT; i++){
       Button_initAndLoad(&playerAmountButtons[i], ctx->renderer, (buttonGap*(i+1))+(buttonW*i), (float)SCREEN_HEIGHT/1.5, buttonW, (float)SCREEN_HEIGHT/7, "Art/ButtonBackground.png", ctx->gFont, "1", 1, (SDL_Color){100,100,100,255});
}
}
