...
bool loadMedia(context* ctx)
{
    ...
    Entity_initPhysics(&ctx->player, SCREEN_WIDTH/2-50, SCREEN_HEIGHT/2-50, 100, 100, 0.8f, 15.0f, 0.3f, .95f, 8.0f, 3);
    Entity_initPhysics(&ctx->testObject, 300, 0, 100, 100, .8f, 12.0f, 0.3f, .95f, 8.0f, 1);
    Entity_init(&ctx->fish, 300, 300, 100, 100, 0.0, 2);
    
    Camera_init(&ctx->camera);
    
    if (!Texture_loadFromFile(&ctx->tilemapSpriteSheet, ctx->renderer, "TileSpritesheet.png"))
    {
    ...
    if (!Entity_setTexture(&ctx->player, ctx->renderer, "StarfishSpriteSheet.png"))
    {
    ...
    if (!Entity_setTexture(&ctx->testObject, ctx->renderer, "RelicArt.png"))
    {
    ...
    if (!Entity_setTexture(&ctx->fish, ctx->renderer, "BackgroundFishSpriteSheet.png"))
    {
    ...
    Tilemap_init(&ctx->tilemap, &ctx->tilemapSpriteSheet, true, 3.75, ctx->tilemap_tilesPerGrid, "/Users/erik/Documents/Projects/CSDL/CSDL/CSDL/map.txt", "SX", 2, "E", 1, ctx->mapEntities, 1);
    
    ctx->fish.clip[0] = (SDL_Rect){0,0,32,32};
    ctx->fish.clip[1] = (SDL_Rect){32,0,32,32};
    BackgroundEntity_init(&ctx->bgFish, &ctx->fish, 8); // CURRENTLY NEED TO INIT AFTER LOADING ENTITY TEXTURE. NEED TO MAKE ENTITY'S TEXTURE A POINTER DONT FORGET
    
    ctx->gFont = TTF_OpenFont("/System/Library/Fonts/Noteworthy.ttc", 56); //Location and font size;
    if(ctx->gFont!= NULL){
        SDL_Color fontCol= {0, 255, 122, 255};
        if(!Texture_loadFromRenderedText(&ctx->fontTexture, ctx->renderer, ctx->gFont, "Starfish Adventures the Epilogue", fontCol))
        {
            printf("Failed to load Font texture!\n");
            success = false;
        }
        fontCol.g = 0; fontCol.b = 0; fontCol.r = 0;
        if (!Button_loadTextures(&ctx->butt, ctx->renderer, "ButtonBackground.png", "Music", ctx->gFont, fontCol))
        {
...
void startGameloop(context* ctx){
    ...
    emscripten_set_main_loop_art(gameLoop, ctx, -1, 1);
}
...
void gameLoop(void *arg){
    ...
    while(SDL_PollEvent(&e))
    {
        ...
        Camera_setCameraOffset(&ctx->camera, ctx->cameraOffsetOffsetx, ctx->cameraOffsetOffsety);
    }
    ...
    Entity_move(&ctx->player, Tilemap_getCollidersAroundEntity(&ctx->tilemap, &ctx->player, &colliderAmount), colliderAmount);
    if (ctx->player.onGround == 1 && checkForLanding){
        ...
        ctx->playerRotation += ctx->player.xVel/2;
    Entity_move(&ctx->testObject, Tilemap_getCollidersAroundEntity(&ctx->tilemap, &ctx->testObject, &colliderAmount), colliderAmount);
    
    
    Camera_followEntity(&ctx->camera, &ctx->player);
    Camera_getObjectOffset(&ctx->camera, &ctx->cameraOffsetX, &ctx->cameraOffsetY);
...
//        sceneObjects[4] = testObject.collider;
    ...
    if (ctx->player.xVel>0) {ctx->lastMovedRight=true;} else if (ctx->player.xVel<0) {ctx->lastMovedRight=false;}
    
    if ((((int)ctx->playerRotation%360 <-195 && (int)ctx->playerRotation%360 > -270) || ((((int)ctx->playerRotation%360 >90 && (int)ctx->playerRotation%360 < 165)))) && !ctx->lastMovedRight && ctx->player.onGround){
        if (!ctx->onNub){
            ctx->currentPlayerSprite = 1;
            ctx->onNub = true;
        }}
    else if((((int)ctx->playerRotation%360 >100 && (int)ctx->playerRotation%360 < 180) || ((((int)ctx->playerRotation%360 <-180 && (int)ctx->playerRotation%360 > -260)))) && ctx->lastMovedRight && ctx->player.onGround){
        if (!ctx->onNub){
    ...
    BackgroundEntity_update(&ctx->bgFish, ctx->renderer, ctx->cameraOffsetX, ctx->cameraOffsetY, ctx->frameCount, 30);
    
    Tilemap_render(&ctx->tilemap, ctx->renderer, ctx->cameraOffsetX, ctx->cameraOffsetY, ctx->cameraOffsetOffsetx, ctx->cameraOffsetOffsety);
    
    Entity_render(&ctx->testObject, ctx->renderer, &ctx->player.clip[0], -1, NULL,  SDL_FLIP_NONE, ctx->cameraOffsetX, ctx->cameraOffsetY);
   
    Entity_render(&ctx->player, ctx->renderer, NULL, ctx->currentPlayerSprite, NULL, SDL_FLIP_NONE, ctx->cameraOffsetX, ctx->cameraOffsetY);
    
    
    //UI RENDERING
    if (!Texture_loadFromRenderedText(&ctx->fpsTexture, ctx->renderer, ctx->gFont, fpsText, ctx->fpsCol))
    {
        printf("Couldn't render fps text!!\n");
    }
    else
    {
        Texture_render(&ctx->fpsTexture,ctx->renderer, NULL, &ctx->fpsLoc, 0.0, NULL, SDL_FLIP_NONE);
    }
...
bool init(context* ctx){
    ...
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO |SDL_INIT_JOYSTICK)<0)
    {
        ...
        ctx->window = SDL_CreateWindow("Erik test", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
        if (ctx->window == NULL)
        {
...
int main(int argc, char * argv[])
{
    ...
    //camera stuff
    ctx.cameraOffsetOffsetx = 0; ctx.cameraOffsetOffsety = 0;
